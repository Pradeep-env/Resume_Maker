...
const Navbar = () => {
    const userOpt =[
        {name: "Dashboard", link: "#", icon: <LayoutDashboard className="text-green-500"/>},
        {name: "Templates", link: "#", icon: <Paperclip className="text-green-500"/>},
        {name: "Articles", link: "#", icon: <Newspaper className="text-green-500"/>},
        {name: "Account", link: "#", icon: <User className="text-green-500"/>},
    ]
  ...
  return (
    <div className="h-[10vh] w-full flex items-center justify-between shadow-md z-10 relative">
      ...
      <div className={`w-[40%] flex items-center justify-evenly flex-col bg-red-100 lg:bg-white transition-all duration-300 ${menu ? 'h-[10vh] mt-0' : 'h-[50vh] mt-[40vh]'} lg:h-full lg:w-[50%] lg:flex-row lg:mt-0 lg:m-0`}>
        ...
        {userOpt.map((opt) => (
          <a href={opt.link} key={opt.name} className="h-full w-full flex items-center lg:justify-between lg:justify-evenly px-2" style={{"display": menu? "none": "flex"}}>
            {opt.icon}
